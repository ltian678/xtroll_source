...
def load_eval_data(dataset_name: str) -> str:
    ...
    if dataset_name == "nato":
        # traindata = load_dataset('wikitext', 'wikitext-2-raw-v1', split='train')
        testdata = load_dataset("tweets", "nontroll1", split="test")
        testdata = "\n\n".join(testdata["text"])
    elif dataset_name == "r":
        testdata = load_dataset("tweets", "nontroll3", split="test")
        testdata = "\n\n".join(testdata["text"])
    elif dataset_name == "cc":
        testdata = load_dataset("tweets", "nontroll2", split="test")
        testdata = "\n\n".join(testdata["text"])
...
def main(
    ...
    *,
    ...
    adapter_path: Path = Path("out/adapter/xtroll-adapter-finetuned.pth"),
    checkpoint_path: Path = Path("checkpoints/xtroll/7B/xtroll.pth"),
    tokenizer_path: Path = Path("checkpoints/xtroll/tokenizer.model"),
    dtype: str = "float32",
